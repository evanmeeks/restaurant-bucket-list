...
import { Coordinates, Venue, VenueSearchResponse, VenueDetailsResponse } from '../../models/venue';
...
function* handleFetchNearbyVenues(
  ...
  try {
    ...
    // Call API - Using V3 service
    const response: VenueSearchResponse = yield call(
      foursquareServiceV3.searchNearbyVenues.bind(foursquareServiceV3),
      coordinates,
    ...
    yield put(
      fetchNearbyVenuesFailure((error as Error).message ?? 'Failed to fetch nearby venues')
    );
  }
}
...
export function* handleFetchRecommendedVenues(
  ...
  try {
    ...
    // Call API - Using V3 service
    const response: VenueSearchResponse = yield call(
      foursquareServiceV3.getRecommendedVenues.bind(foursquareServiceV3),
      coordinates,
      limit
    );
...
function* handleSelectVenue(action: PayloadAction<string>) {
  try {
    ...
    if (venue) {
      ...
      // Otherwise fetch from API - Using V3 service
      const venueDetails: Venue = yield call(
        foursquareServiceV3.getVenueDetails.bind(foursquareServiceV3),
        venueId
      );
    ...
    // Could add one if needed (e.g., fetchVenueDetailsFailure)
  }
}
